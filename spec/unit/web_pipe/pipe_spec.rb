...
RSpec.describe WebPipe::Pipe do
  ...
  describe "#operations" do
    it "maps plug names with resolved operations" do
      ...
      expect(operations.map { |name, op| [name, op.()] }).to eq([[:one, "one"], [:two, "two"]])
    end
  end
  ...
  describe "#to_proc" do
    it "returns the kleisli composition of all the plugged operations" do
      ...
      expect(to_proc.(build_conn).response_body).to eq(["one"])
    end
  end
  ...
  describe "#call" do
    it "behaves like a rack application" do
      ...
      expect(pipe.(default_env)).to eq([200, {}, ["Hello, world!"]])
    end
